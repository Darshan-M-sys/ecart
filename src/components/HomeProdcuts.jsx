...
const HomeProduct = () => {
  ...
  useEffect(() => {
    const categories = ["kitchen-accessories"]; // Add more as needed
    ...
    const fetchAllCategories = async () => {
      try {
        ...
        const combinedProducts = allData.flat();
 ...
 const [range,setRange]=useState([0,Infinity])
  const filtered = products.filter((item) =>(
    item.title.toLowerCase().includes(query))&&(
      item.price*10/2>=range[0] && item.price*10/2 <= range[1]
    )
  );
 ...
 <span className="search-icon"><span><CiSearch /> </span> </span>
          ...
          <div className="header-login-container"><h3>About</h3></div>
          <div className="header-login-container"><h3>Contact</h3></div>
          <div className="header-login-container">
            <span><CgProfile /></span><h3>Login</h3>
          </div>
        </div>
        <br /><br />
      </header>
          ...
          return (
            <div className="product-card" key={index}>
              <Link to={`/allProductsDe/${item.id}`}>
                <img src={item.thumbnail} className="product-images" alt={item.title} />
              </Link>
              ...
              <div className="rating-container">
                <p>
                  Rating: {item.rating} &nbsp;
                  <span className="ratings">{renderStars(item.rating)}</span>
                </p>
